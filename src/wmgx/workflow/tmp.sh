...
#SBATCH -J merge_PRJNA526861
#SBATCH --partition=main
#SBATCH -t 5:30:00  # 4 hours, adjust if needed
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH -o /cfs/klemming/home/x/xueyaw/xueyao/data/metagenomics/PRJNA526861/log/%x_%A_%a.log
#SBATCH -e /cfs/klemming/home/x/xueyaw/xueyao/data/metagenomics/PRJNA526861/log/%x_%A_%a.err
...
MERGED_DIR="/cfs/klemming/home/x/xueyaw/xueyao/data/metagenomics/$BIOPROJECT/3_biobakery_merged_tmp"
...
while read -r SAMPLE_ID; do
    METAPHLAN_TABLE="${BASE_DIR}/${SAMPLE_ID}/metaphlan/main/${SAMPLE_ID}_taxonomic_profile.tsv"


    if [ -f "$METAPHLAN_TABLE" ]; then
        METAPHLAN_ABSOLUTE_ABUNDANCE_OUTPUTS+="$METAPHLAN_TABLE "
...
while read -r SAMPLE_ID; do
    ...
    cp $BASE_DIR/${SAMPLE_ID}/humann/main/${SAMPLE_ID}_genefamilies.tsv  $TMP_MERGED_HUMANN_genefamilies_rpk
    cp $BASE_DIR/${SAMPLE_ID}/humann/relab/genes/${SAMPLE_ID}_genefamilies_relab.tsv  $TMP_MERGED_HUMANN_genefamilies_relab
...
for norm in "rpk" "relab" "cpm"; do
    INPUT_FILE="${MERGED_DIR}/humann_${norm}/${BIOPROJECT}_genefamilies_${norm}.tsv"

    # Skip if input file is missing
    if [ ! -f "$INPUT_FILE" ]; then
        echo "❌ Error: Missing $INPUT_FILE, skipping..."
        continue
    fi
    ...
    for group in "${!groups[@]}"; do
        ...
        # Regroup the table
        humann_regroup_table --input "$INPUT_FILE" --groups "$group" --output "${OUTPUT_BASE}.tsv"
