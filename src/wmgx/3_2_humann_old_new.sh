...
HM_LOG="${ANALYSIS_HM_DIR}/preprocess.log"
CPUS=20
...
humann_single_end_sample_id() {
    ...
    local HM_LOG="${ANALYSIS_HM_DIR}/preprocess.log"
    local LOG_FILE="$ANALYSIS_HM_DIR/${SAMPLE_ID}/${SAMPLE_ID}_kneaddata_single_humann_temp/${SAMPLE_ID}_kneaddata_single.log"


    # Check if "Output files created" exists in the log file
    if [ -f "$LOG_FILE" ]; then
        if grep -q "Output files created" "$LOG_FILE"; then
            echo "Output files already created for $SAMPLE_ID, skipping this sample." >> $HM_LOG
            return
        fi
    fi
...
humann_paired_end_sample_id() {
    ...
    local HM_LOG="${ANALYSIS_HM_DIR}/preprocess.log"
    local LOG_FILE="$ANALYSIS_HM_DIR/${SAMPLE_ID}/${SAMPLE_ID}_kneaddata_paired_merged_humann_temp/${SAMPLE_ID}_kneaddata_paired_merged.log"
