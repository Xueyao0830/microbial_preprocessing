...
SAMPLE_KD_CSV="/proj/naiss2024-6-169/users/x_xwang/data/metagenomics/$BIOPROJECT/2_2_merged_kd/PRJEB37017_kneaddata_remove_low_reads_merge_samples.tsv"
...
SAMPLE_IDS=$(awk -F, 'NR>1 {print $1}' "$SAMPLE_KD_CSV")
...
# Loop through SAMPLE IDs and Library Layouts sequentially
# Loop through the SAMPLE_IDs and find corresponding LAYOUT from the CSV_FILE
while IFS= read -r SAMPLE_ID; do
    # Find the corresponding row in CSV_FILE that matches the SAMPLE_ID and get the LAYOUT (column 16)
    LAYOUT=$(awk -F, -v id="$SAMPLE_ID" '$1 == id {print $16}' "$CSV_FILE")
...
python $SRC_UTILS/merge_metaphlan_table_absolute.py $METAPHLAN_ABSOLUTE_ABUNDANCE_OUTPUTS > "$ANALYSIS_MP_DIR/${BIOPROJECT}_merged_absolute_abundance_kneaddata.csv"
