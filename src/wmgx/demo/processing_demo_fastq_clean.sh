...
#SBATCH --mem=300G
#SBATCH --exclusive
#SBATCH -o /proj/naiss2024-6-169/users/x_xwang/project/microbiota_crc/log/process/test_process_clean_%j.log
#SBATCH -e /proj/naiss2024-6-169/users/x_xwang/project/microbiota_crc/log/process/test_process_clean_%j.err
...
SRR_IDS=$(ls $INPUT_FASTQ_DIR/*_{1,2}_sed.fastq | xargs -n 1 basename | cut -d '_' -f 1 | sort | uniq)
...
process_single_end_srr_id() {
    ...
    kneaddata --unpaired $INPUT_FASTQ_DIR/${SRR_ID}_1_sed.fastq \
...
process_paired_end_srr_id() {
    local SRR_ID=$1
    mkdir -p $OUTPUT_DIR/${SRR_ID}
    kneaddata --input1 $INPUT_FASTQ_DIR/${SRR_ID}_1_sed.fastq \
              --input2 $INPUT_FASTQ_DIR/${SRR_ID}_2_sed.fastq \
              --reference-db $REFERENCE_DB/GRCh38_index \
              --output $OUTPUT_DIR/${SRR_ID}
}
...
process_srr_id() {
    local SRR_ID=$1
    if [ -f ${INPUT_FASTQ_DIR}/${SRR_ID}_2.fastq ]; then
...
#sbatch metaphlan_clean_demo.sh
